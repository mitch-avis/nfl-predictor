...
`.agents/TODO.md` (above all "Roadmap Status", Milestone 60 task 60.9 and its open
question and acceptance items, and task 56.7) and this file.

## State (written 2026-09-25, Milestone 60 after closing 60.8)

- Branch `feat/m60-cli-consolidation`, cut from `main` at `703ea25`; version `0.27.1`. Not
  pushed; merging and pushing are must-ask. `scripts/gate.sh` exits `0` on the final tree (1020
  passed, coverage 92.15%; `--web` last run green at `0.27.0`, and nothing under `web/` or
  `nfl_predictor/api/` changed since). The working tree is clean after the `0.27.1` commits.
- Tasks 60.1-60.8 are done and archived (`ARCHIVE.md`, "Milestone 60 (partial)"); task 58.5 is
  ...
  signed-off copy.
- `scripts/` holds only `gate.sh`, which CI now runs. Every web job runs `<python> -m
  nfl_predictor <command>`. `README.md` and `AGENTS.md` use the front door;
  `tests/test_readme_commands.py` parses every README command.
...
1. Task 60.9: the `compare` command (paired comparison of two walk-forward runs from their fold
   checkpoints; see the TODO text for the windows and columns); it must reproduce an existing
   `REVIEW.md` rescore exactly before it is trusted.
2. Each chunk small, versioned, gated (`--web` when the web code changes) and committed; rewrite
