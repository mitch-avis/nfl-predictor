...
- Branch `feat/m60-cli-consolidation`, cut from `main` at `703ea25`; version `0.21.0`. Not
  pushed; merging and pushing are must-ask. `scripts/gate.sh --web` exits `0` on the final
  tree (see the `0.21.0` commits). The working tree is clean. (The user deleted their
...
- 60.6 is in progress; its "Progress" note in `TODO.md` lists what landed and what is next.
  ...
  --live`), and left `scripts/<name>.py` as thin shims for the web jobs.
- The characterization snapshots must not change during a move; an intended output change
  ...
  `0.21.0` renamed module keys and added the front door and `--live`, to 216).
...
1. Continue 60.6 with the shared options module (`nfl_predictor/cli/options.py` exists with
   two helpers), the renames with aliases (PROPOSAL.md section 3), the
   remaining flag removals (`leakage_audit --include-market`, `weekly_run --wf-n-jobs`), the
   YAML default, and the step-2 follow-ups (`wf_compare`
