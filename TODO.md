...
Goal: make walk-forward (WF) evaluation **checkpoint after each candidate** (and optionally per fold)
so that long runs:

- visibly make progress (persistent artifacts update)
- can be safely resumed after interruption without losing completed work
- reuse cached results when inputs/config are identical

Rationale:

- WF comparisons can take many hours or days when evaluating multiple settings.
- Weekly operation and off-season sweeps require robust checkpoint/restart behavior.
- `--resume` should skip already-computed WF candidates based on stable keys + dataset fingerprint.

### Design constraints (must hold)

- Checkpoint writes must be **atomic** (no corrupted partial artifacts).
- Resume must be **idempotent** (re-running does not change results unless inputs/config changed).
- No leakage: checkpointing must not change fold construction or evaluation semantics.
- Artifact format must be stable (Parquet/JSON) and human-inspectable.

---

### Tasks

#### 38.1 - Define stable candidate keys + fingerprints

- [ ] Identify where WF candidates are enumerated (likely under `nfl_predictor/ml/walk_forward.py`
  and/or orchestration in `scripts/weekly_run.py`).
- [ ] Define a stable `candidate_key` string that uniquely identifies a WF candidate, including:
  - model kind / feature window / calibration mode
  - market mode / prob source / blend method
  - uncertainty setting
  - market weight + clamp (if applicable)
  - any XGBoost override knobs that materially change results
- [ ] Define a dataset fingerprint used for caching/resume:
  - file path + mtime + size OR a fast content hash (preferred) for `--data-path`
  - include critical WF args (eval seasons, start week, calibration weeks, include-postseason, etc.)
- [ ] Add a small helper in a shared location (e.g. `nfl_predictor/utils/fingerprints.py`) to compute:
  - `dataset_fingerprint`
  - `wf_run_fingerprint` (dataset + args + code version marker if desired)

Acceptance:

- [ ] Candidate keys are stable across machines and runs.
- [ ] Fingerprints change when inputs/config change and remain constant otherwise.

---

#### 38.2 - Per-candidate checkpoint artifact (resume primitive)

- [ ] Create a WF output directory under the run dir:
  - `models/<run_id>/wf_compare/`
- [ ] After each candidate finishes, write a per-candidate artifact:
  - filename: `wf_candidate_<candidate_key>.parquet` (or `.json`)
  - contents: per-fold metrics + candidate metadata
- [ ] Implement atomic write:
  - write to `<path>.tmp`, then `os.replace(tmp, final)`
- [ ] On `--resume`, skip candidates whose artifact exists and validates:
  - non-empty
  - contains required columns/metadata
- [ ] If an artifact exists but is invalid/corrupt:
  - rename to `.corrupt.<timestamp>` and recompute the candidate

Acceptance:

- [ ] Killing the process mid-run preserves completed candidate artifacts.
- [ ] Restarting with `--resume` skips completed candidates reliably.

---

#### 38.3 - Aggregated WF progress table (visibility + quick comparisons)

- [ ] Maintain a single summary table in `models/<run_id>/wf_compare/`:
  - `wf_summary.parquet` (preferred) and/or `wf_summary.csv`
- [ ] After each candidate completes:
  - compute a single summary row using the project’s selection hierarchy fields:
    - brier/logloss/reliability
    - pool metrics
    - margin/total MAE (tie-breakers)
    - market-relative metrics if present
  - append/update the summary table atomically
- [ ] Ensure the summary table includes:
  - candidate_key
  - dataset_fingerprint / wf_run_fingerprint
  - wall time + timestamp
  - “rank” fields if available (or leave ranking to post-processing)

Acceptance:

- [ ] While WF is running, `wf_summary.*` updates after each candidate.
- [ ] You can sort/filter `wf_summary` to see the current best candidate without waiting for the
  full run.

---

#### 38.4 - Optional: per-fold heartbeat checkpoint (nice-to-have)

- [ ] Add an optional flag (default OFF):
  - `--wf-checkpoint-per-fold` (boolean)
- [ ] When enabled, write a tiny JSONL or Parquet row after each fold:
  - `wf_fold_progress.jsonl` containing (candidate_key, season, week, metric snapshot, timestamp)
- [ ] Make sure fold checkpointing is low overhead and never blocks training.

Acceptance:

- [ ] Long candidates still show frequent progress even if they run >60 minutes.
- [ ] Disabling the flag yields no extra fold writes.

---

#### 38.5 - Wire checkpointing into `scripts/weekly_run.py` stage 1

- [ ] Ensure Stage 1 (“WF comparison”) uses the new checkpoint/resume logic:
  - enumerate candidates
  - for each candidate:
    - skip if completed (per-candidate artifact exists and matches fingerprint)
    - run evaluation
    - write per-candidate artifact + update summary table
- [ ] Ensure `--resume` behavior:
  - does not recompute completed candidates
  - continues from the next missing candidate
- [ ] Add stable INFO logs:
  - `WF candidate i/N starting: <key>`
  - `WF candidate i/N done in XmYs: brier=..., logloss=...`

Acceptance:

- [ ] Stage 1 resumes cleanly after interruption with no manual cleanup.
- [ ] Logs make it obvious that progress is occurring.
...
- [ ] Add unit tests for:
  - candidate_key stability (same inputs -> same key)
  - dataset fingerprint stability (same file -> same fingerprint)
  - resume skipping: if candidate artifact exists, evaluation function is not called
  - corrupt artifact handling: corrupt file triggers recompute
- [ ] Add an integration-ish test using a tiny synthetic dataset:
  - run WF for 2 candidates
  - simulate interrupt by stopping after first candidate (call function once)
  - rerun with resume and confirm only the second candidate runs

Acceptance:

- [ ] `pytest` covers checkpoint/resume logic and passes reliably.

---

#### 38.7 - Documentation updates

- [ ] Update `README.md` “Weekly pipeline” section:
  - mention Stage 1 WF emits `wf_compare/` artifacts and is resumable
- [ ] Update `AGENTS.md`:
  - instruct agents to avoid deleting `wf_compare/` during active runs
  - mention how to resume and how to inspect `wf_summary.*`

Acceptance:

- [ ] Docs clearly explain where progress artifacts live and how to resume.

---

Acceptance (Milestone 38 complete)

- [ ] Stage 1 WF writes per-candidate artifacts and an aggregated summary table.
- [ ] `--resume` continues without recomputing completed candidates.
- [ ] A forced kill/restart (SIGINT) preserves all completed work and finishes correctly after restart.
