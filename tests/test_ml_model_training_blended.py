"""Tests for blended margin/total training."""
...
def test_train_blended_margin_total_model_full(monkeypatch) -> None:
    ...
    class DummyBlendModel:
        """Dummy blend model for testing."""
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: DummyBlendModel(np.array([0.6, 0.4])),
    )
    monkeypatch.setattr(
        ml_model_training,
        "get_market_baseline",
        lambda frame: (
            ...
            frame["market_total_line"].to_numpy(dtype=float),
        ),
    )
    monkeypatch.setattr(
        ml_model_training,
        "_predict_home_win_prob",
        lambda _margin, _cal: np.full(len(_margin), 0.6),
    )
    ...
    assert model.team_model.margin_model == "margin_model"
    assert model.tuned_params["team"] == {"max_depth": 2}
